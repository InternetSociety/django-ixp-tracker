...
def test_as_not_registered_to_zz_returns_true(faker):
    es = EventStore(IXP_TRACKER_EVENT_MAP, MemoryEventStore())
    asn = create_asn(faker, es)

    assert as_zz_country_check(asn.number, asn.country_code, asn.nro_status)


def test_as_registered_to_zz_and_not_assigned_returns_false(faker):
    es = EventStore(IXP_TRACKER_EVENT_MAP, MemoryEventStore())
    asn = create_asn(faker, es, country_code="ZZ", nro_status=NROStatus.AVAILABLE)

    assert not as_zz_country_check(asn.number, asn.country_code, asn.nro_status)


def test_as_registered_to_zz_but_assigned_returns_true(faker):
    es = EventStore(IXP_TRACKER_EVENT_MAP, MemoryEventStore())
...
def test_as112_returns_true(faker):
    es = EventStore(IXP_TRACKER_EVENT_MAP, MemoryEventStore())
