...
from ixp_tracker.data_lookup import (
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Do a full reset of ASNs rather than incremental update",
        )
        parser.add_argument(
            "--backfill",
            type=str,
            default=None,
            help="The month you would like to backfill data for",
        )
    ...
    def handle(self, *args, **options):
        try:
            ...
            data_lookup: AdditionalDataSources = (
                load_lookup(IXP_TRACKER_DATA_LOOKUP_FACTORY)
                or DefaultAdditionalDataSources()
            )
            ...
            if backfill_date is None:
                ...
                processing_date = datetime.strptime(backfill_date, "%Y%m").replace(
                    tzinfo=timezone.utc
                )
                if reset:
                    logger.warning(
                        "The --reset option has no effect when running a backfill"
                    )
